...
function appendAutomaticDevLog(target = "posts") {
  ...
  const content = [
    ...
    `1. 发布时间：${iso}`,
    `2. 发布模块：${label}`,
    `3. 数据文件：${getSettings().path || "data/posts.json"}`,
    `4. 发布方式：${RA_IS_LOCAL_ADMIN ? "本地后台提交并推送 GitHub" : "外网后台通过 Worker 写入 GitHub"}`,
    "",
    "## 2. 后台操作步骤",
    "",
    "1. 打开后台 admin.html。",
    ...
    "4. 确认本次新增或修改内容已经保存。",
    "5. 点击“保存并发布”。",
    "6. 等待后台状态栏提示正在等待公开博客上线。",
    "",
    "## 3. 本地维护时建议执行的命令",
    "",
    "进入项目目录：",
    "",
    "    cd /Users/galph/Documents/MyPersionWeb",
    "",
    "检查工作区：",
    "",
    "    git status --short",
    ...
    "",
    "    node --check admin.js",
    "    node --check local-admin-server.mjs",
    ...
    "",
    "    git add .",
    `    git commit -m \"Update ${target} content\"`,
    "    git push origin main",
    "",
    "然后打开 GitHub Actions：",
    "",
    "    https://github.com/Galphrui/ralphrong/actions",
    "",
    "确认最新的 Deploy to GitHub Pages 任务是绿色对勾。",
    "",
    "## 5. 前台确认方式",
    "",
    "部署成功后打开线上页面，并使用 Command + Shift + R 强制刷新。",
    "",
    "常用地址：",
    "",
    "    文章：https://galphrui.github.io/ralphrong/#posts",
    "    代码库：https://galphrui.github.io/ralphrong/#code",
    "    工具库：https://galphrui.github.io/ralphrong/#tools",
    "    开发日志：https://galphrui.github.io/ralphrong/#devlogs",
    "",
    "## 6. 失败时先排查",
    "",
    "1. GitHub Actions 是否红叉。",
    "2. data/posts.json 是否已经更新。",
    "3. 浏览器是否需要强制刷新。",
    "4. 如果是附件问题，检查 public-assets/tools 目录是否有文件。",
    "5. 如果是页面空白，先执行本地构建命令查看报错。",
    "",
    "## 7. 本次结果",
    "",
    "本次发布流程已经提交。最终是否上线，以后台状态栏、GitHub Actions 绿色对勾、线上页面刷新后的内容为准。",
  ].join("\n");
